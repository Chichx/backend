...
const renderProducts = (products) => {
    ...
    products.forEach((product) => {
      ...
      productBox.innerHTML = `
        ...
        <h3 class="font-medium text-xl text-white text-center">${product.title}</h3>
        ...
        <div class="w-full h-[1px] bg-gray-500 my-4"></div>
        <button class="bg-red-500 hover:bg-black/20 transitions-all duration-200 w-full py-2 rounded-xl text-gray-200 font-medium" onclick="deleteProduct('${product.id}')">
          Eliminar
        </button>
      `;
...
document.getElementById("newProductForm").addEventListener("submit", (e) => {
    ...
    if (codeDuplicado) {
        ...
        const newProduct = { title, description, price, code, stock, thumbnail };
